...
static int camera_init(int module, int width, int height, bool is_mipi)
{
	...
	CHECK_COMMAND(v4l2_set_format(clipper_id, width, height,
				      V4L2_PIX_FMT_YUV420));
	CHECK_COMMAND(v4l2_set_crop(clipper_id, 0, 0, width, height));
	CHECK_COMMAND(v4l2_set_format(sensor_id, width, height,
				      V4L2_MBUS_FMT_YUYV8_2X8));
	if (is_mipi)
		CHECK_COMMAND(v4l2_set_format(nxp_v4l2_mipicsi, width, height,
					      V4L2_MBUS_FMT_YUYV8_2X8));
	CHECK_COMMAND(v4l2_reqbuf(clipper_id, BUFFER_COUNT));
	return 0;
}

static int camera_run(int module, int width, int height, bool is_mipi,
		      SurfaceComposerClient *client)
{
	...
	sp<SurfaceControl> surfaceControl = client->createSurface(String8::format("CameraNTest%d", module),
								  width, height, HAL_PIXEL_FORMAT_YV12,
	...
	clipper_id = module == 0 ? nxp_v4l2_clipper0 : nxp_v4l2_clipper1;
...
static void *camera_thread(void *data)
{
	...
	ALOGD("camera thread: module %d, %dX%d, is_mipi %d, client %p",
	      p->module, p->width, p->height, p->is_mipi, p->client);
